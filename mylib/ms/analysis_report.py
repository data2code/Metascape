...
def _report(myopt, dbcon=None):
    ...
    REF_METASCAPE='Tripathi S. et al., Meta- and Orthogonal Integration of Influenza "OMICs" Data Defines a Role for UBR4 in Virus Budding. Cell Host & Microbe (2015) 18:723-735.'
    ...
    if opt['IMG_GO_HEATMAP']:
        ...
        s_sum+="""
              ...
              <div class="panel panel-info">
                <div class="panel-heading">Figure %d. Heatmap of enriched terms across input gene lists, colored by p-values.</div>
    ...
    def get_links(png_url, gene_list_name_ppi=None):
        ...
        cytoscape_js_url=""
        cytoscape_js_url = (
                           '/gp/' if IS_METASCAPE_DOT_ORG  else '') + 'Content/cytoscape_js_client/index.html?session_id=@SESSION_ID@'
        linksHtml += """
                        @ONLINE_BEGIN@
                        <a href='{0}{1}' title='interactive cytoscape' target='_blank'>
                        <img class='link' src='{2}' >
                        </a>
                        @ONLINE_END@
                    """.format(cytoscape_js_url,
                               gene_list_name_in_html,
    ...
    if t_go is not None and len(t_go)>0:
        ...
        if opt['LISTS']>1:
            t2=t2[['_PATTERN_','GO','Category','Description','#GeneInGOAndHitList','%InGO','LogP','Log(q-value)']].copy()
        else:
            t2=t2[['GO','Category','Description','#GeneInGOAndHitList','%InGO','LogP','Log(q-value)']].copy()
        t2.rename2({'#GeneInGOAndHitList':'Count', '%InGO':'%', 'LogP':'Log10(P)','Log(q-value)':'Log10(q)'})
        if opt['LISTS']>1:
            for idx in t2.index:
                s_pat=t2.ix[idx, '_PATTERN_'][1:]
                t2.ix[idx, '_PATTERN_']=" ".join(['<div style="background-color:%s;width:20px;height:20px;float:left;"></div>' % (S_color[i] if ((x in c_order) and (s_pat[c_order[x]]=="1")) else "#FFFFFF") for i,x in enumerate(S_name)])
        i_tbl+=1
        s="""
          <div class="panel panel-info">
    ...
    if ('PPI' in opt) and (not opt['PPI']['disablePPI']): #user ppi checked:
        ...
        s_ppi+=("For each given gene list, protein-protein interaction enrichment analysis has been carried out with the following databases: %s. "+\
            ...
            % (", ".join(S_ds), opt['PPI']['minSize'], opt['PPI']['maxSize'], i_ref)
...
#                    % (i_fig)
            ...
            def get_top3(list_name,isMCODE):
                ...
                for i, r in df1.iterrows():
                    ...
                    for ann in r['Annotation'].split(';'):
                        ...
                        rows.append({
                            'Color': color,
                            'MCODE':r['Network'].replace(list_name+'_SUB1_',''),
                            'GO':t[0],
                            'Description':t[1],
                            'Log10(P)':t[2],
                        })
            ...
            for gene_list_name in all_gene_list:
                ...
                for i, nw in enumerate(nws):
                    ...
                    if opt['LISTS']>1:
                        ...
                        colored_by_html += '(Keep MOCDE Nodes Only)' if isMCODE else '(Full Connection)'
    ...
    s_body="\n".join([s_sum, s_ovlp, s_ann, s_mem, s_enrh, s_ppi, s_gpec, s_cytoscape_js_client])
...
def report(myopt, dbcon=None):
    ...
    s_offline=re.sub('@ICON_URL@', 'icon', x.sub('', re.sub('@URL_BASE@', '.', s)))
    s_offline="""
<!DOCTYPE html>
<html lang="en">
  <head>
...
if __name__=="__main__":
    ...
    #t_lists.ix[0,'Unique']=5000
    #t_lists.ix[1,'Unique']=4000
