...
class Cache(object):
    ...
    def load(tax_id=9606, l_use_GPDB=True, user_db=None):
        ...
        if Cache.C_PATHWAY is None:
            ...
            if os.path.isfile(s_panther):
                t=pd.read_csv(s_panther, names=['A','B','C','D','E'])
                for i in t.index:
                    s=t.ix[i,'B']
...
class EntrezGene(object):
    ...
    def __init__(self, user_db = None, l_use_GPDB=False, tax_id=None):
        ...
        (self.C_GENENEW, self.C_GENEDESCRIPTION, self.C_SPECIES, self.C_GENEMAP)=Cache.get(l_use_GPDB=self.GPDB, tax_id=self.tax_id, user_db=self.user_db)
    ...
    def GP_annotation(self, S_gene_id, S_ann=None):
        ...
        c_map={t_map.ix[i,'ID']:t_map.ix[i,'NAME'] for i in t_map.index}
        c_name={t_map.ix[i,'ID']:t_map.ix[i,'DISPLAY'] for i in t_map.index}
        for i in S_ann:
            ...
            if not c_map[i].startswith('Ontology_'):
                ...
                for k in tmp.index:
                    s_gene=tmp.ix[k, 'Gene']
                    S_terms=tmp.ix[k, 'term_ids'].split(",")
                    ...
                    tmp.ix[k, 'term_ids']="; ".join(list(tmp2.term_name))
    ...
    def GP_gene_sarray_by_affyid_sarray(self, S_affy):
        ...
        c={t.ix[i,'source_id']:str(t.ix[i,'gid']) for i in t.index}
    ...
    def GO_description(self, go_id):
        ...
        return {'type':t.ix[0,'TYPE'], 'name':t.ix[0,'NAME'], 'description':t.ix[0,'DESCRIPTION']}
    ...
    def all_GO_gene(self, toTaxID=None):
        ...
        if self.GPDB:
            ...
            data=[]
            for i in t.index:
                S_gene=t.ix[i,'gids'].split(',')
                data.append(pd.DataFrame({'GO_ID': [t.ix[i,'GO_ID']]*len(S_gene), 'GENE_ID':S_gene, 'TAX_ID':t.ix[i,'tax_id']}))
    ...
    def GO_descript_by_gene(self, gene_id, s_type="BP", nTop=3):
        ...
        for i in range(len(t)):
          S.append(t.ix[i,'GO_ID']+"("+t.ix[i,'TYPE']+"):"+t.ix[i,'NAME'])
        s="; ".join(S)
    ...
    def parent_GO(self, go_id, l_recursive=False):
        if self.GPDB:
            s_dist='distance>0' if l_recursive else 'distance=1'
            t=self.db.from_sql("SELECT tt.parent_term_id GO_ID,t.term_name NAME,MIN(tt.distance) DISTANCE FROM term2term tt,term t WHERE tt.parent_term_id=t.term_source_id AND tt.term_id=? AND "+s_dist+" order by distance,term_id group by tt.parent_term_id,t.term_name order by distance,parent_term_id", [go_id])
        else:
            s_dist='DISTANCE>0' if l_recursive else 'DISTANCE=1'
    ...
    def id_conversion(self, S_source_id, target_tax_id=None, Source_tax_id=None, s_source_type=None):
        ...
        if type(Source_tax_id) is not list:
        ...
        LEFT JOIN homologene h1 ON h1.gid = gs.gid AND h1.tax_id IN ({0})
        LEFT JOIN homologene h2 ON h1.homologene_id = h2.homologene_id AND h2.tax_id IN ({1})
        ...
        WHERE gs.source_id IN (""".format(",".join([str(x) for x in Source_tax_id]), target_tax_id),
        """)
        AND t.id_type_name IN ({0})
        AND gs.id_type_id = t.id_type_id""".format("'"+"','".join(S_source_type)+"'"),
        S_source_id)
        for i in t.index:
            if pd.isnull(t.ix[i, 'homologene_gid']) and (int(t.ix[i, 'tax_id'])==target_tax_id):
                t.ix[i, 'homologene_gid']=int(t.ix[i, 'gid'])
                t.ix[i, 'homologene_tax_id']=target_tax_id
        ...
        t=t3.merge(t2, left_on='source_id', right_on='source_id')
        t.sort_values(['InputID','source_id', 'priority4','priority1', 'priority3','priority2'], ascending=[True, True, False, True, False, False], inplace=True)
        t.drop_duplicates('InputID', inplace=True)
        t=t[['InputID','homologene_gid']]
    ...
    def gene_by_symbol(self, s_symbol, toTaxID=None, l_keep_all=False):
        ...
        if len(t)>0: return str(t.ix[0,'GENE_ID']) if not l_keep_all else util.iarray2sarray(t['GENE_ID'])
        return None
    ...
    def gene_sarray_to_table(self, S_gene, l_description=True):
        ...
        for i in range(len(S_gene)):
            if S_gene[i] and S_gene[i] in self.C_GENENEW:
                T.ix[i,'Symbol']=self.C_GENENEW[S_gene[i]] or ''
                if l_description: T.ix[i,'Description']=self.C_GENEDESCRIPTION[S_gene[i]] or ''
        return T
    ...
    def go_by_gene(self, gene_id, s_keyword=""):
        t=None
        if not s_keyword:
            t=self.db.from_sql("SELECT GO.GO_ID,GO.NAME,GO.TYPE,MIN(GG.DISTANCE) DISTANCE FROM N_GENE_GO G,E_GO2GO GG,E_GO GO WHERE G.GO_ID=GG.CHILD_GO_ID AND GG.PARENT_GO_ID=GO.GO_ID AND G.GENE_ID=? and GO.NAME like '%"+s_keyword+"%' GROUP BY TYPE,GO.GO_ID ORDER BY TYPE,DISTANCE,GO.GO_ID", params=[gene_id])
        return t
    ...
    def protein_accession_to_gene_id(self, s_IPI, toTaxID=None, l_ortholog=True):
        ...
        if len(t_ann):
            ...
            if len(t):
                ...
                S=self.ortholog(t_ann.ix[0,'N_GENE_ID'], toTaxID)
                return S or None
        return None
    ...
    def all_gene_names(self, taxID=None):
        ...
        for i in range(len(t_ann)):
            c_ann[str(t_ann.ix[0,'GENE_ID'])]=t_ann.ix[i,'SYMBOL']
        return c_ann
    ...
    def pathway_label(self, s_gene):
        ...
        if self.GPDB:
            ...
            tmp=tmp[tmp.GO_ID.apply(lambda x: x in self.C_PATHWAY)].copy()
        else:
        ...
        S_GO=util.sarray2sarray(tmp['GO_ID'])
        for j in range(len(S_GO)):
            S_GO[j]=self.C_PATHWAY[S_GO[j]]
        S_GO.sort()
        return "; ".join(S_GO)
