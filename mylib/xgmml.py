...
''' % (s_name, now)
    ...
    def print_xgmml(self, t_noa, t_sif, s_idCol, s_nameCol, S_nodeCol, S_edgeCol, l_remove_loop=False):
        ...
        for i in range(len(t_noa)):
            ...
            s+="  <node label=\""+s_name+"\" id=\""+c_id[s_gene]+"\">\n"
            s+="    <att type=\"string\" name=\"canonicalName\" value=\""+s_name+"\"/>\n"
            for s_col in S_name:
                if s_col in S_nodeCol:
                    #in S_listCol or c_type[s_col]!='string':
                    v=t_noa[s_col].iloc[i]
                    v='NA' if pd.isnull(v) else str(v)
                    s+="    <att type=\""+c_type[s_col]+"\" name=\""+s_col+"\" value=\""+escape(str(t_noa[s_col].iloc[i]))+"\"/>\n"
                #else:
        ...
        for i in range(len(t_sif)):
            ...
            for s_col in S_name:
                if s_col in S_edgeCol:
                    ...
                    v='NA' if pd.isnull(v) else str(v)
                    s+="    <att type=\""+c_type[s_col]+"\" name=\""+s_col+"\" value=\""+escape(str(t_sif[s_col].iloc[i]))+"\"/>\n"
            s+="  </edge>\n"
        s+="</graph>"
        return s
...
class Network(object):
    ...
    def create_node(self):
        ...
        self.T_node=pd.DataFrame({'Gene':pd.Series(S_nodes)})

    def create_edge(self):
        rows=[]
        for k,v in self.data.items():
            for n in v.keys():
                if k<=n: rows.append({'Gene_A':k, 'Gene_B':n, 'TYPE':'Direct', 'SCORE':v[n]})
        self.T_edge=pd.DataFrame(rows)

    def __init__(self, x=None, allow_indirect=False, name='Untitled', T_node=None, s_noa=None):
        ...
        if isinstance(x, Network):
            ...
            self.T_edge=x.copy()
            # fix order of Genes in T_edge
            for i in self.T_edge.index:
                # swap ID if needed
                if self.T_edge.ix[i,'Gene_A']>self.T_edge.ix[i,'Gene_B']:
                    tmp=self.T_edge.ix[i,'Gene_A']
                    self.T_edge.ix[i,'Gene_A']=self.T_edge.ix[i,'Gene_B']
                    self.T_edge.ix[i,'Gene_B']=tmp
            ...
            if (ext==".sif"):
                ...
                self.name=xg.name
        if len(self.T_edge)>0 and 'Indirect' in list(self.T_edge['TYPE']):
    ...
    def are_neighbors(self, node1, node2):
        if node1 not in self.data: return False
        if node2 not in self.data[node1]: return False
        return True
    ...
    def delete_node(self, node):
        ...
        del self.data[node]
        for s in S:
            del self.data[s][node]
        self.T_node=self.T_node[self.T_node['Gene']!=node]
        self.T_edge=self.T_edge[(self.T_edge['Gene_A']!=node) & (self.T_edge['Gene_B']!=node)]
        for s in S:
            if s in self.data and len(self.data[s])==0:
                del self.data[s]
                self.T_node=self.T_node[self.T_node['Gene']!=s]
                self.T_edge=self.T_edge[(self.T_edge['Gene_A']!=s) & (self.T_edge['Gene_B']!=s)]
        if self.allow_indirect and 'Indirect' not in self.T_edge['TYPE']:
    ...
    def subnetwork(self, S_node, l_keep_as_singleton=False):
        ...
        c_keep=set(S_node)
        data2={}
        #for node1,nbs in self.data.items():
        #    if node1 not in c_keep: continue
        #    for node2 in nbs.keys():
        #        if node2 not in c_keep: continue
        for node1 in S_node:
            for node2 in self.neighbors(node1):
                if node1>node2: continue # keep in case equal
                if node2 not in c_keep: continue
                if node1 not in data2: data2[node1]={}
                if node2 not in data2: data2[node2]={}
                data2[node1][node2]=self.data[node1][node2] # transfer score
                data2[node2][node1]=self.data[node1][node2] # transfer score
        c_new=set()
        if l_keep_as_singleton:
            for node1 in S_node:
                if node1 not in data2:
                    data2[node1]={}
                    data2[node1][node1]=1.0
                    c_new.add(node1)
        t=Network(data2)
        if not t.is_empty():
            t.T_node=pd.merge(t.T_node, self.T_node, left_on='Gene', right_on='Gene', how='left')
            c_header={ x for x in self.T_edge.header() if x not in ['Gene_A','Gene_B','TYPE']} # 'SCORE' column might be shared
            S_dup=[x for x in t.T_edge.header() if x in c_header ]
            if len(S_dup):
                t.T_edge.drop(S_dup, axis=1, inplace=True)
            t.T_edge=pd.merge(t.T_edge, self.T_edge, left_on=['Gene_A','Gene_B','TYPE'], right_on=['Gene_A','Gene_B','TYPE'], how='left')
            if l_keep_as_singleton and len(c_new)>0:
                t.T_edge['SCORE']=t.T_edge['SCORE'].apply(lambda x: 1.0 if pd.isnull(x) else x)
        return t
    ...
    def combine_network(self, others=None):
        ...
        for another_net in others:
            for k in another_net.data:
                if k not in self.data:
                    self.data[k]={}
                for k2,v in another_net.data[k].items(): # yzhong 07292013
                #for k2,v in another_net.data[k]:
                    if k2 not in self.data[k]:
                        self.data[k][k2]=v
                    else:
                        self.data[k][k2]=max(v, self.data[k][k2]) # yzhong 07292013
                        #self.data[k][k2]=max(v, self.data[k])
        data1=[self.T_node]
        data2=[self.T_edge]
        for another_net in others:
        ...
        self.T_edge.sort_values(['Gene_A','Gene_B','TYPE'], inplace=True)
