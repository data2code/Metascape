...
def sql_in(s_sql_left, s_sql_right, S_id, num=1000, con=None, params_before=None, params_after=None):
    if con is None:
    ...
    if db_type(con)=='ORACLE':
        ...
        if len(t):
            ...
            return from_sql(con, s_sql_left+s_id+s_sql_right, params=params_before+S_id+params_after)
        #else:
        #    t=[]
    ...
    if db_type(con) =='MYSQL' and not S:
        if l_NOT:
            ...
            s_id = "("+s_col+" IN (''))"
    t=from_sql(con, s_sql_left+s_id+s_sql_right, params=params_before+S_id+params_after)
    return t
...
def sql_in_old2(s_sql_left, s_sql_right, S_id, num=1000, con=None, params_before=None, params_after=None):
    if con is None:
        util.error_msg('Database connection is not provided!')
    S_id=list(S_id)
    n=len(S_id)
...
class DB:
    ...
    def from_sql(self, sql, params=(), index_col=None, coerce_float=True, l_select=None):
        ...
        if db_src=='MYSQL':
            ...
            while True:
                try:
                    return from_sql(self.con(), sql, params=params, index_col=index_col, coerce_float=coerce_float, l_select=l_select)
                except Exception as e:
                    if 'connect' in e.args[1].lower() and i_try<=n_try:
    ...
    def sql_in(self, s_sql_left, s_sql_right, S_id, num=1000, params_before=None, params_after=None):
        return sql_in(s_sql_left, s_sql_right, S_id, num=num, con=self.con(), params_before=params_before, params_after=params_after)
    ...
    def table_exists(self, s_table):
        return table_exists(self.con(), s_table, s_db=self._name)
