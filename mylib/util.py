...
def unix(exe, l_print=True, l_error=True, l_block=True):
...
def df2sdf(df, s_format='%.2g', s_null='',l_all_str=False):
    ...
    for s in header(df2):
        if df[s].dtype != np.dtype(object):
            ...
            elif np.issubtype(df[s].dtype, np.flexible):
                df2[s]=[s_null if pd.isnull(x) else s_format % x for x in df[s]]
            else:
...
def sample_table(l_text=False, nrows=10, ncols=5):
    ...
    if ncols<=26:
        ...
        util.error_msg('Cannot handle more than 702 columns for now')
...
class Spotfire:
    ...
    def new_dxp(template_file, output_file, data_file):
        ...
        for x in zin.infolist():
            ...
            if is_python3():                      #YfZ, 2/2/2018
                #maybe install chardet will help to know how data is encoded
                data= data.decode(errors='ignore')     #program runs fine, but spotfire can't open the dxp file
