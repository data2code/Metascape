...
class EUtils(object):
    ...
    def __init__(self, def_args=None, debug=False):
        ...
        self.def_args={'retmode': 'xml', 'usehistory': 'y', 'retmax': 250, 'email': '', 'tool':''}
    ...
    def esearch(self, args):
        ...
        out=self._fetch('/esearch.fcgi',
            dict(args.items()+[('retmax',retmax),('retstart',retstart)]))
    ...
    def _batch_retrieve(self, action, args, count=0, func=None, retmax=10000):
        ...
        #    del args['cmd']
        out=self._fetch("/"+action+'.fcgi', dict(args.items()+[('retmax',retmax),('retstart',0)]))
        ...
        while (count>0 and retstart+retmax < count):
            ...
            out=self._fetch("/"+action+'.fcgi', dict(args.items()+[('retmax',retmax),('retstart',retstart)]))
            if func is not None and callable(func):
                out=func(out)
            S_xml.append(out)
        return S_xml
...
if __name__=='__main__':
    ...
    if True:
        id,args=eu.esearch({'db':'pubmed', 'term':'Genomics[Affiliation] AND Foundation[Affiliation] AND 2014[pdat]'})
        print args
