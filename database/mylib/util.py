...
def unix(exe, l_print=True, l_error=True):
    '''Execute a unix command and return results in a list\nExample: unix(["ls", "-l"]
    exe: str, command line string
    ...
    if err is not None and len(err)>0 and l_error:
        print("ERROR> "+"".join(err))
    if l_print:
        print(out)
    else:
        if is_python3():
            return out.decode('utf-8') # + '***' + str(err) #out.splitlines()
        else:
            return str(out)
...
def read_csv(s_file, *args, **kwargs):
    ...
    kw=kwargs.copy()
    #if 'keep_default_na' not in kw:
    #    kw['keep_default_na']=False
    if not hasattr(s_file, "endswith"):
        return pd.read_csv(s_file, *args, **kw)
    elif s_file.endswith('.gz'):
        import gzip
        return pd.read_csv(gzip.open(s_file, 'rb'), *args, **kw)
    elif s_file.endswith('.zip'):
        from zipfile import ZipFile
        zz = ZipFile(open(s_file, 'rb'))
        return pd.read_csv(zz.open(zz.namelist()[0]), *args, **kw)
    else:
        return pd.read_csv(s_file, *args, **kw)
...
def df2sdf(df, s_format='%.2g', s_null='',l_all_str=False):
    ...
    for s in header(df2):
        if df[s].dtype != np.dtype(object):
            ...
            elif np.issubdtype(df[s].dtype, np.float):
...
def split(df, n_chunk=1, chunk_size=0):
    ...
    if portrait:
        ...
        for i in range(nRow):
            ...
            for j in range(nCol):
                ...
                S.append('      <TD'+s_td+'>')
                S.append('&nbsp;' if (pd.isnull(df.values[i,j]) or str(df.values[i,j])=='') else str(df.values[i,j]))
                S.append('</TD>\n')
            S.append('    </TR>\n')
        S.append('  </TBODY>\n')
    else:
        for i in range(nCol):
            ...
            th=tag2str(callback(th.copy(), -1, i, S_header[i], df)) if callback is not None else s_th
            ...
            for j in range(nRow):
                ...
                merge_tag(clr, td)
                s_td=tag2str(callback(clr, j, i, S_header[i], df)) if callback is not None else tag2str(td)
                S.append('    <TD'+s_td+'>')
                S.append('&nbsp;' if (pd.isnull(df.values[j,i]) or str(df.values[j,i])=='') else str(df.values[j,i]))
                S.append('</TD>\n')
            S.append('  </TR>\n')
    S.append('</TABLE>')
    return "".join(S)
...
def sample_table(l_text=False):
    ...
    t=pd.DataFrame(np.random.randn(10,5), columns=list('ABCDE'))
    if l_text:
        t['S']=list('abcdefghij')
    return t
...
def col_type(df, s_col):
    ...
    if np.issubdtype(df[s_col].dtype, np.float): return "r"
...
def summary(df, n_sample=0):
    ...
    if n_sample>0 and n_sample<len(df):
        t=df.ix[df.index[np.random.permutation(len(df))[:n_sample]]]
    else:
        t=df
    S_type=col_types(t)
    S_min=t.min()
...
"""
        if not os.path.exists(template_file):
            util.error_msg('Template dose not exist: %s!' % template_file)
        ...
        for x in zin.infolist():
            ...
            if x.filename == s_target_file:
                ...
                if m:
                    ...
                    util.error_msg('Tag for FilePath is not found in %s!' % s_target_file);
...
class StopWatch(object):
    ...
    def check(self, s_msg, l_reset=True):
        ...
        x=time.time()
        dur=x-self.start
        print(self.prompt+"Passed: %.1f secs, %s" % (dur, s_msg))
...
if __name__=="__main__":
    ...
    def callback(tag, r, c, col, df):
        ...
        if r>-1 and col=='Discontinued':
            tag['style']= 'background-color: #999999;' if df.ix[r,c] else 'background-color:#af89dc;'
        return tag
